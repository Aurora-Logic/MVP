...
function buildBankFooterHtml(bc) {
    ...
    return `<div style="margin-top:28px;padding-top:18px;border-top:1px solid #e4e4e7"><div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:${bc || '#18181b'};margin-bottom:8px">Bank Details</div><table style="border-collapse:collapse">${tbl}</table></div>`;
}
...
function buildMinimalTpl(p, c, bc, t, rows, secs, logo, title, num, isInv) {
    ...
    if (rows.length) { h += `<div style="font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#18181b;margin:24px 0 10px">${isInv ? 'Items' : 'Pricing'}</div>`; h += buildPricingHtml(rows, c, t, bc, 'minimal'); }
    if (p.paymentTerms) h += `<div style="font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#18181b;margin:24px 0 8px">Terms</div><div style="color:#71717a;font-size:12px;line-height:1.8">${editorJsToHtml(p.paymentTerms, p)}</div>`;
    h += buildBankFooterHtml(bc);
    return h;
}
