...
function buildModernTpl(p, c, bc, t, rows, secs, logo, title, num, isInv) {
    ...
    let h = logo;
    h += `<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px"><div><h1 style="font-size:24px;font-weight:800;color:#18181b;letter-spacing:-.5px;margin:0 0 4px">${title}${ver}</h1><div style="font-size:13px;color:#71717a">${num} \u00B7 ${fmtDate(p.date)}</div></div><div style="text-align:right;padding:10px 16px;border-radius:6px;background:${bc}0D"><div style="font-size:10px;text-transform:uppercase;letter-spacing:.8px;font-weight:700;color:${bc}">${isInv ? 'Amount Due' : 'Estimated Total'}</div><div style="font-size:20px;font-weight:800;color:${bc};font-family:var(--mono)">${c}${t.grand.toLocaleString(c === '₹' ? 'en-IN' : 'en-US')}</div></div></div>`;
